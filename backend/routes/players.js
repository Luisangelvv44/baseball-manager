...
router.get('/team-stats', async (req, res) => {
  try {
    ...
    const { stats: rawStats } = await computeSeasonStats(season.id, { teamId: USER_TEAM_ID });
    ...
    const stats = rawStats.map((p) => ({
      ...
      pitching: p.pitching && {
        ...
        w: p.pitching.w,
        l: p.pitching.l,
        ip: p.pitching.ip.toFixed(1),
        era: p.pitching.era != null ? p.pitching.era.toFixed(2) : null,
        so: p.pitching.so,
        bb: p.pitching.bb,
        whip: p.pitching.whip != null ? p.pitching.whip.toFixed(2) : null,
      },
    }));
